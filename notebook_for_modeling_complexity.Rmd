...
This notebook is for estimating the average weight for newly released games.
...
First we'll look at the distributions of our outcome variables.
...
```{r designers complexity, fig.height=10, fig.width=10, warning=F, message=F}
...
```{r publishers complexity, fig.height=15, fig.width=10, warning=F, message=F}
...
We can see how well the penalized model does in resampling.
...
```{r predicted vs actual for valid set, warning=F, message=F}
...
preds_year%>%
        ...
        select(Row, Outcome, ID, Game, Published, Actual, glmnet, xgbTree) %>%
        mutate_if(is.numeric, round, 2) %>%
        flextable() %>%
        flextable::autofit() %>%
...
preds_year%>%
        ...
        flextable::bg(., j = c("Actual", "glmnet", "xgbTree"),
                      bg = weight_func) %>%
        set_caption(paste(params$end_training_year+4, " - Estimated Complexity from Models"))
...
# models obj
models_obj_avgweight = avgweight_models

# save
readr::write_rds(games_datasets, file = paste("deployment/games_datasets_avgweight_", Sys.Date(), ".Rdata", sep=""))
readr::write_rds(prepped_recipe_avgweight, file = paste("deployment/prepped_recipe_avgweight_", Sys.Date(), ".Rds", sep=""))
readr::write_rds(models_obj_avgweight, file = paste("deployment/models_obj_avgweight_", Sys.Date(), ".Rds", sep=""))
