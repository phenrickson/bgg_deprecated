...
save(ratings_models, file=paste("ratings_models", Sys.Date(), ".Rds", sep="_"))
...
```{r make coef plot for stan, warning=F, message=F, fig.height=8, fig.width=8}
...
Here we can see the issue with the model fit to the geek rating - we tend to overpredict games that are stuck at the initial starting value of 5.5, and then unpredict games that have moved away. Since we care more about the rank moreso than the precise geek rating, we'll see how this goes. The average looks to be in better shape.
...
Looks like we're getting a pretty similar fit between the the linear model we fit with stan and the lightly penalized model here.
...
How did these models do in tuning during resampling? 
...
While we don't have coefficients from these nonparametric models, we can get a sense of the importance of specific features by looking at the permutation importance.
...
#### 2021 Geek Rating Predictions Chart (stan lm)
...
#### 2021 Geek Ratings Predictions Chart (ranger)

```{r plot full 2021 game with ranger, fig.height=50, fig.width=8, warning=F, message=F}
...
```{r now plot the same table but with the prediction intervals, warning=F}

# stan lm
preds_2022_stan = posterior_preds_and_actual %>%
        filter(yearpublished == 2022) %>%
        filter(outcome_type == 'baverage') %>%
        group_by(outcome_type, game_id, yearpublished, name, average, baverage) %>%
        summarize_at(vars(value), funs(!!!p_funs)) %>%
        mutate(outcome = case_when(outcome_type == 'average' ~ average,
                                   TRUE ~ baverage)) %>%
        ungroup() %>%
        rename(.pred_lower = perc_5,
               .pred = perc_50,
               .pred_upper = perc_95) %>%
        mutate(method = 'stan_lm') %>%
        select(-average, -baverage, -perc_10, -perc_20, -perc_80, -perc_90)
 
# ranger
preds_2022_rf =test_preds %>%
        select(outcome_type, data, stan_lm_preds, glmnet_preds, ranger_preds, xgbTree_preds, ranger_interval_preds) %>%
        unnest() %>%
        select(outcome_type, yearpublished, game_id, name, outcome, .pred, .pred_lower, .pred_upper) %>%
        filter(yearpublished == 2022) %>%
...
# combine
# preds_2022_rf %>%
#         bind_rows(., preds_2022_stan) %>%
#         rename(Pred = .pred,
#                Pred_Up = .pred_upper,
#                Pred_Low = .pred_lower) %>%
#         mutate(method = case_when(method == 'stan_lm' ~ 'lm',
#                                   TRUE ~ 'rf')) %>%
#         mutate_if(is.numeric, round, 2)
...
#### 2022 Geek Rating Predictions Chart (stan lm)
...
#### 2022 Geek Rating Predictions Chart (ranger)

```{r plot full 2022 game baverages, fig.height=10, fig.width=8, warning=F, message=F}
...
```{r save test preds, warning=F,}

save(test_preds, file=paste("rating_preds_", Sys.Date(), ".Rdata", sep=""))
save(oos_preds, file = paste("oos_rating_preds", Sys.Date(), ".Rdata", sep = ""))

```


```{r save csvs, warning=F}
...
save(test_preds_out, file = paste("test_preds_out", Sys.Date(), ".Rdata", sep=""))
save(oos_preds_out, file = paste("oos_preds_out", Sys.Date(), ".Rdata", sep=""))
