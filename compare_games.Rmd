...
pca_predict<-pca_map %>%
  ...
  mutate(method = "pca") %>%
  select(dataset, method, training, pca) %>%
  left_join(., data_combined,
            by = "method") %>%
  mutate(pca_rotation = map2(data, pca,
                        ~ predict(.y, newdata =.x))) %>%
  mutate(pca_rotation_names = map2(.x = pca_rotation,
                                   .y = data,
                                   ~ cbind.data.frame(.x, .y))) %>%
...
```{r plot new games published in this space, fig.height=4}

pca_predict[1,]$viz_ind

pca_predict[2,]$viz_ind

```

Now extract the neighbors

```{r extract the neighbors from this, warning=F}
<<<<<<< HEAD

new_neighbors<-pca_predict %>%
  select(dataset, neighbors) %>% 
  unnest() %>%
  mutate(similarity = 100*1/(1+ sqrt(dist))) %>%
  select(dataset, game_id, name, neighbor_game_id, neighbor_name, similarity, dist, dist_rank) %>%
...
new_recommend %>%
  arrange(dataset, rec_rank) %>%
  filter(name == 'Unfathomable')

new_recommend %>%
  arrange(dataset, rec_rank) %>%
  filter(name == 'Anno 1800') %>%
    mutate_if(is.numeric, round, 2) %>%
  rename(compare_by = dataset) %>%
  select(compare_by, name, neighbor_name, rec_score, rec_rank) %>%
   arrange(compare_by, rec_rank) %>%
...
fwrite(new_recommend, file = paste("game_recommendations_data/", Sys.Date(), ".csv", sep=""))
=======
...
fwrite(paste("game_recommendations_data/", Sys.Date(), ".csv", sep=""))


>>>>>>> b30e92b223c43b6643c152f31cdc10c431315dff
