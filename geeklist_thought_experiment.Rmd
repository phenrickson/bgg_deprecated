---
title: What would the BGG Geek list look like if games were all published in the same year?
output:
  html_document:
    df_print: paged
---

Using our model previously trained to predict the geek rating (using games published through 2019), what would happen if we predicted all games under the assumption that they were released in the same year? I arbitrarily set 2015 as the year of release for all games, then ran them through the previously trained models.
...
## Predicted Geek List vs Actual Geek List

```{r get predicted vs plot in plot, fig.height=10, fig.width=10}
...
hypothetical_preds %>%
  ...
  rename(GeekRating = outcome,
         ...
         ModelRank = pred_rank,
         ModelRating = mean) %>%
  select(ID, Game, GeekRating, GeekRank, ModelRank, ModelRating) %>%
