...
This notebook is for building predictive models of the boardgamegeek community's average and geek ratings in order to predict which upcoming games are likely to be highly rated. This notebook contains only the code for training and deploying models; exploratory analysis with a greater exploration of the data is contained in a different notebook.
...
We now will explore the game ratingss and the relationship between ratings and features we have in the dataset. At this stage in the process, we will have restricted ourselves to looking at our training set (games published prior to 2019) to get a basic understanding of both our outcome variable and the features.
...
We'll assemble our training dataset to start, meaning we will start with games published before 2019 and then pull in the other datasets to create features at the game level.
...
```{r set quantiles for geek ratings}

# quantiles
baverage_quantiles = quantile(games_train$baverage, seq(0, 1, .01)) %>%
        as.data.frame() %>%
...
# average quanties
average_quantiles = quantile(games_train$average, seq(0, 1, .01)) %>%
        as.data.frame() %>%
        rownames_to_column("perc") %>% 
        set_names(., c("perc", "value")) %>%
        mutate(perc = gsub("%", "", perc)) %>%
...
quantile_dummy = bind_rows(baverage_quantiles %>%
...
Example: Terra Mystica. This is a game I'd expect the model to be pretty high on for both outcomes.
...
#samp_sims %>%
   #     ggplot(., aes(x=value))+
        geom_histogram(data = samp_sims,
                       aes(x=value),
                       bins = 100,
                 alpha = 0.7,
                 fill = 'grey60',
                 color = '#F0F0F0')+
        geom_vline(data = samp_sims,
        aes(xintercept=outcome),
        alpha=0.9,
        col = "blue")+
        geom_text(data = samp_sims,
        aes(x=outcome),
...
```{r look at any game 2, warning=F, message=F, fig.height=4, fig.width=8}
...
```{r look at any game seafall, warning=F, message=F, fig.height=4, fig.width=8}
...
First Martians. I'm expecting the model to overrate this one, as it's pretty complex and has a ton of mechanics, but has been poorly received.
...
```{r look at any game dark souls, warning=F, message=F, fig.height=4, fig.width=8}
...
```{r partial dependence from ranger, fig.height=10, fig.width=10, warning=F, message=F}
...
partials_ranger_baverage %>%
        ...
        mutate(variable = factor(variable,
          ...
          labs(caption = str_wrap(paste("Centered individal conditional expectation plots for top predictors from ranger. Model trained on games published prior to", params$end_training_year, "with at least 200 user ratings.", 125)))
...
predictors_loop = top_predictors_xgbTree %>% filter(outcome_type == 'baverage') %>% pull(Variable)

# loop over top predictors for xgbTree
j=2
...
partials_xgbTree_baverage %>%
        ...
        mutate(variable = factor(variable,
          ...
          xlab("Feature Value")+
          ylab("Effect on Geek Rating")+
          labs(caption = str_wrap(paste("Centered individal conditional expectation plots for top predictors from xgbTree. Model trained on games published prior to", params$end_training_year, "with at least 200 user ratings.", 125)))
...
Let's look at the out of sample predictions during training from the tree based ensemble methods. How do they compare?
...
The tree based ensemble models are generally highly correlated, which is about what I would expect. It looks like the boosted trees are more willing to be aggressive in predicting the geek rating though, so I'll be curious to see how they compare on the validation set.
...
### Validation Set: `r params$end_training_year + 1`
...
```{r predicted vs actual for 2019, fig.height=10, fig.width=8, warning=F, message=F}
...
What do we make of all this? The boosted trees look to be performing the best for the geek rating overall. But I want to see how they perform over the distribution of games. Are the boosted trees just picking up accuracy by accurately predicting 5.5 for bottom tier games?

```{r subpopulation results, fig.height=10, fig.width=10, warning=F, message=F}
...
test_preds %>%
        ...
        rename(method = variable,
               pred = value) %>%
        group_by(outcome_type) %>%
        mutate(strata = cut(outcome, breaks = 4, labels =F)) %>%
        ...
        theme_phil()+
        my_caption+
        xlab("Stratified Outcome Variable")
        
```

Hmm. So it would seem that isn't the case, the boosted trees generally perform the best across the board. Given the distribution of this variable, it also isn't a surprise to me that the models perform 'better' on the lower end of games. One thing to consider is whether we should just be predicting a categorical variable with labels for 'top game', 'bad game', rather than trying to directly estimate the rating.
...
```{r plot full game baverage, fig.height=80, fig.width=8, warning=F, message=F}
...
### Test Set: `r params$end_training_year+2` And On

We'll now do the same thing, predicting the next years except we don't really know the 'true' geek rating for these games as user ratings are still coming in for these games. We can compare the predictions with the actual, but we'll have to interpret it with some caution.
...
```{r plot full 2020 game baverage, fig.height=65, fig.width=8, warning=F, message=F}
...
#### Predicting `r params$end_training_year + 4`
...
quantile_func<- function(x) {
  ...
  breaks<-quantile(x, probs = seq(0, 1, by=.25), na.rm=T) %>% as.vector()
#  breaks = weight_deciles
  colorRamp=colorRampPalette(c("red", "white", "deepskyblue1"))
  col_palette <- colorRamp(length(breaks))
  mycut <- cut(x, 
...
preds_year%>%
        ...
        flextable::bg(., j = c("Mean", "glmnet", "ranger", "stan_lm", "xgbTree"),
                      bg = quantile_func) %>%
        set_caption(paste(params$end_training_year+4, " - Predicted Geek Rating from Models"))
...
```{r plot full 2022 game baverage, fig.height=10, fig.width=8, warning=F, message=F}
...
plot_obj %>%
       ggplot(., aes(x=reorder(name, mean),
                     ...
                     y = .pred,
                      ymin = .pred_lower,
                      ymax = .pred_upper))+
        geom_pointrange(fatten=2, position = position_dodge(width = 0.75)) +
        facet_wrap(yearpublished ~ outcome_type,
                   scales = "free")+
      #  scale_x_reordered()+
        coord_flip()+
...
```{r save predictions, warning=F, message=F}
...
# models obj
trained_models_obj = ratings_models

# save
readr::write_rds(games_datasets, file = paste("deployment/games_datasets_", Sys.Date(), ".Rdata", sep=""))
readr::write_rds(prepped_recipe_ratings, file = paste("deployment/prepped_recipe_ratings_", Sys.Date(), ".Rds", sep=""))
readr::write_rds(trained_models_obj, file = paste("deployment/trained_models_obj_", Sys.Date(), ".Rds", sep=""))
