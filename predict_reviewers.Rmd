...
Where do games rank overall?


```{r look at games by rank}
...
test_probs_with_actual %>%
    ...
    select(-.row, -actual, -variable) %>%
      rename(pred = value) %>%
  group_by(username) %>%
  do(data.frame(., newval = ecdf(.$pred)(.$pred))) %>%
  rename(perc = newval) %>%
  mutate_if(is.numeric, round, 2) %>%
  select(-pred) %>%
 group_by(game_id, name) %>%
  ...
  select(-SD) %>%
    select(Method, Outcome, ID, Published, Game, Rank, Mean, mrbngr, everything()) %>%
    mutate_if(is.numeric, round, 2) %>%
    flextable() %>%
    flextable::autofit()  %>%
    bg(., j = c("Gyges", "mrbngr",
                ...
                "Quinns", "rahdo", "TomVas", "WtchItPly", "ZeeGarc"),
       bg = perc_func) %>%
          set_caption(paste("Percentile Ranks by Reviewer for Games Published in 2021"))
...
And percentiles for 2022.

```{r percentiels for 2022 by reviewer, warning=F, message=F}
...
```{r cosmic frog, warning=F, message=F}
