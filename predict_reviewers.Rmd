...
We can zoom in on a few of those comparisons in particular.

```{r compare reviers, warning=F, message=F}
...
col_num_func<- function(x) {
  ...
  breaks<-seq(1, 8, by = 1)
#  breaks = weight_deciles
  colorRamp=colorRampPalette(c("white", "blue3"))
  col_palette <- colorRamp(length(breaks))
  mycut <- cut(x, 
    breaks = breaks,
    include.lowest = TRUE, 
...
```{r rating variance, warning=F, message=F}
...
Now place individuals in here.

```{r place reviewers, message=F, fig.height=8, eval = F}

## Quinns
set.seed(1999)
pca_data %>%
  mutate(Quinns_Has_Owned = case_when(have_owned_Quinns == 1 ~ 'yes',
                                      TRUE ~ 'no')) %>%
  ggplot(., aes(x=PC01,
                color = Quinns_Has_Owned,
                label = name,
                y=PC02))+
  ...
  facet_wrap(Quinns_Has_Owned~.)

## rahdo
set.seed(1999)
pca_data %>%
  mutate(rahdo_Has_Owned = case_when(have_owned_rahdo == 1 ~ 'yes',
                                      TRUE ~ 'no')) %>%
  ggplot(., aes(x=PC01,
                color = rahdo_Has_Owned,
                label = name,
                y=PC02))+
  geom_jitter(width=0.4,
              height = 0.1)+
  ...
  theme(legend.title = element_text())+
  scale_color_manual(values = c("grey60", "navy"))+
  facet_wrap(rahdo_Has_Owned~.)


## TomVasel
set.seed(1999)
pca_data %>%
  mutate(TomVasel_Has_Owned = case_when(have_owned_TomVasel == 1 ~ 'yes',
                                      TRUE ~ 'no')) %>%
  ggplot(., aes(x=PC01,
                color = TomVasel_Has_Owned,
                label = name,
                y=PC02))+
  geom_jitter(width=0.4,
              height = 0.1)+
  ...
  theme(legend.title = element_text())+
  scale_color_manual(values = c("grey60", "navy"))+
  facet_wrap(TomVasel_Has_Owned~.)

## ZeeGarcia
set.seed(1999)
pca_data %>%
  mutate(ZeeGarcia_Has_Owned = case_when(have_owned_ZeeGarcia == 1 ~ 'yes',
                                      TRUE ~ 'no')) %>%
  ggplot(., aes(x=PC01,
                color = ZeeGarcia_Has_Owned,
                label = name,
                y=PC02))+
  geom_jitter(width=0.4,
              height = 0.1)+
  ...
  theme(legend.title = element_text())+
  scale_color_manual(values = c("grey60", "navy"))+
  facet_wrap(ZeeGarcia_Has_Owned~.)

## Gyges
set.seed(1999)
pca_data %>%
  mutate(Gyges_Has_Owned = case_when(have_owned_Gyges == 1 ~ 'yes',
                                      TRUE ~ 'no')) %>%
  ggplot(., aes(x=PC01,
                color = Gyges_Has_Owned,
                label = name,
                y=PC02))+
  geom_jitter(width=0.4,
              height = 0.1)+
  ...
  theme(legend.title = element_text())+
  scale_color_manual(values = c("grey60", "navy"))+
  facet_wrap(Gyges_Has_Owned~.)



```

Can also look at publishers.
...
```{r get coefs from the models, warning =F, fig.height=10}
...
training_preds_glmnet = all_models %>%
...
```{r evalaute 2020 preds, fig.height=8}
...
```{r specify thresholds and compute}
...
pred_record %>%
  filter(outcome_type == 'own') %>%
  select(game_id, yearpublished, game_id, name, username, glmnet, outcome_type, glmnet) %>%
  mutate(method = "glmnet") %>%
  mutate(game_id = as.character(game_id),
         yearpublished = as.character(yearpublished)) %>%
  rename(Outcome = outcome_type,
  ...
  rename(Prob = glmnet) %>%
  arrange(desc(Prob)) %>%
    select(Method, Outcome, ID, Published, Game, User, Prob) %>%
      mutate_if(is.numeric, round, 2) %>%
    flextable() %>%
    flextable::autofit() %>%
    bg(., j = c("Prob"),
